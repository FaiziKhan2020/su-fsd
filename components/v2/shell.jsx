...
function Avatar({ user, size = "md" }) {
  if (!user) return null;
  const px = size === "sm" ? 20 : size === "lg" ? 40 : 26;
  return (
    <span className="person-av" style={{ width: px, height: px, fontSize: px*0.4, background: user.color }}>
      {user.initials}
    </span>
  );
}
...
function TopBar({ crumbs, onNewIdea }) {
  return (
    <div className="topbar">
      ...
      <button className="btn sm primary" onClick={onNewIdea}>+ New idea</button>
    </div>
  );
}

window.RD2 = Object.assign(window.RD2 || {}, { Avatar, StagePill, HealthDot, BrandChip, Sidebar, TopBar });
