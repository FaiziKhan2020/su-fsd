...
// ===== Pipeline Board (with stream-aware cards) =====
...
function ProductCard({ p, onOpen }) {
  ...
  return (
    <button onClick={() => onOpen(p.id)} className="product-card"
      ...
      {waiting && p.stage !== "approval" && <div style={{ fontSize: 11, color: "var(--warn)", marginTop: 4 }}>⏳ Waiting on {waiting.name}</div>}
      {p.streams && (
        <div className="streams">
          {["sourcing","design","listing"].map(k => {
            ...
            return (
              <div className="stream-row" key={k} data-status={s.status}>
                <span className="lbl">{k.slice(0,3)}</span>
...
window.RD2 = Object.assign(window.RD2 || {}, { IdeaInbox, NewIdeaModal, Board, ProductCard });
